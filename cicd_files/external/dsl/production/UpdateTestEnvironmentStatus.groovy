...
def pipelineBeingGeneratedName = 'RPT-RC_Update-Test-Environment-Status'

pipelineJob(pipelineBeingGeneratedName) {
    description(commonSteps.defaultJobDescription(pipelineBeingGeneratedName,
        "This Job is to update the status of a test environment in RPT.",
        "cicd_files/external/dsl/production/UpdateTestEnvironmentStatus.groovy",
        "cicd_files/external/jenkins/production/UpdateTestEnvironmentStatus.Jenkinsfile"
    ))

    parameters {
        stringParam(commonParams.customerSlave())
        stringParam(externalParams.envName())
        stringParam(externalParams.status())
    ...
    definition {
        cpsScm {
            ...
            scriptPath('cicd_files/external/jenkins/production/UpdateTestEnvironmentStatus.Jenkinsfile')
        }
    }
}
